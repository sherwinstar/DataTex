    ...
    , ui(new Ui::Settings)
{
    ...
    ui->NotesPath->setText(DataTex::CurrentDocumentDatabasePath);
...
//    ui->LanguageSelect->setCurrentIndex(ui->LanguageSelect->findData(DataTex::currentlanguage,Qt::UserRole,Qt::MatchExactly));
    ...
    ui->ThemeSelect->addItem("Linux Breath",QVariant("linux_breath"));
    ui->ThemeSelect->addItem("Light",QVariant("light"));
    ui->ThemeSelect->addItem("Dark",QVariant("darkstyle"));
    ui->ThemeSelect->setCurrentIndex(ui->ThemeSelect->findData(currentTheme,Qt::UserRole,Qt::MatchExactly));
    ...
    connect(ui->savePasswordDDB,&QPushButton::clicked,this,[&](){
        ...
        fdb_encription.exec(QString("UPDATE Notes_Folders SET UserName = '%1' WHERE FileName = '%2'")
                                .arg(ui->DocDatabaseUserName->text(),db));
        fdb_encription.exec(QString("UPDATE Notes_Folders SET PassWord = '%1' WHERE FileName = '%2'")
                                .arg(Password,db));
    });
...
//        QString user = SqlFunctions::Get_String_From_Query(
//            QString("SELECT UserName FROM Notes_Folders WHERE FileName = '%1'").arg(notefolder),DataTex::DataTeX_Settings);
//        QString pass = SqlFunctions::Get_String_From_Query(
//            QString("SELECT PassWord FROM Notes_Folders WHERE FileName = '%1'").arg(notefolder),DataTex::DataTeX_Settings);
